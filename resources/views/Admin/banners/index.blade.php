    ...
    <div class="row breadcrumbs-top">
     <div class="breadcrumb-wrapper col-12">
      <ol class="breadcrumb">
       ...
       <li class="breadcrumb-item"><a href="{{ route('admin.roles.create') }}">Create Banner</a>
       </li>
     ...
     <div class="card-body card-dashboard">
      ...
      <table class="table table-striped table-bordered dataex-res-configuration">
       ...
       <tbody>
        ...
        <tr>
         <td>{{ ++$key }}</td>
         <td><img src="{{ asset('assets/img/banners/'.$banner->image) }}" class="img-thumbnail" alt=""></td>
         <td>
            <span class="badge badge-{{ $banner->status === true ? "success" : "danger" }}">{{ $banner->status === true ? "ON" : "OFF" }}</span>
            <form action="{{ url('/admin/banners/statusChange/'.$banner->id) }}" method="post">
                @csrf
                <input type="hidden" name="status" value="{{ $banner->status === true ? false : true }}">
